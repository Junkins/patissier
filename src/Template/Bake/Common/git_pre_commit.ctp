...
foreach ($files as $file) {
    ...
    // http://cs.sensiolabs.org/
    ob_start();
    passthru(sprintf('./vendor/bin/php-cs-fixer fix %s %s -v --ansi', escapeshellarg($file), $cmdArg), $return);
    $output = ob_get_clean();
    if ($return != 0) {
...
passthru('cd operation;../vendor/bin/phpunit;', $operationErrorCode);
passthru('cd environment/migrations;../../vendor/bin/phpunit;', $migrationsErrorCode);
